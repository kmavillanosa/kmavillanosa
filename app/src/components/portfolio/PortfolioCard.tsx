...
function PortfolioCard({ page }: PortfolioCardProps) {
	...
	return (
		...
		>
			<Card 
				className="h-full flex flex-col hover:shadow-2xl transition-all duration-300 cursor-pointer transform hover:-translate-y-2 bg-white border-gray-200 overflow-hidden"
			>
				{/* Card thumbnail - uniform height, prioritize iframe, then image, then link preview */}
				<div className="h-48 w-full overflow-hidden bg-gray-100 relative flex-shrink-0">
					{previewIframe ? (
						<iframe
						...
						<div className="h-full w-full flex flex-col items-center justify-center bg-gradient-to-br from-purple-50 to-blue-50 p-4">
							...
							<p className="text-xs text-gray-600 text-center break-all line-clamp-2">
				...
				<div className="p-5 flex-1 flex flex-col">
					<h5 className="text-xl font-bold tracking-tight text-gray-900 group-hover:text-purple-600 transition-colors mb-2 line-clamp-2">
						{page.title}
					</h5>
					{page.description && (
						<p className="font-normal text-gray-600 line-clamp-3 mb-3 flex-1 text-sm">
							{page.description}
						</p>
					)}
					{page.date && (
